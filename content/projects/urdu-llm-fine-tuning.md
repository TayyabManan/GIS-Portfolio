...
description: "Qwen 2.5 7B Instruct fine-tuned with QLoRA for Urdu script, Roman Urdu, and Urdu/English code-mixing. The current version wins 79.5% of blind pairwise comparisons against the base model across two independent LLM judges, adds RAG-aware training, and ships as a live Gradio + Modal demo, all for about $60."
...
**[Read the full write-up →](/blog/building-urdu-llm-fine-tuning)**
...
## What it does
- One model handles Urdu script, Roman Urdu (Latin-script Urdu, how most Pakistanis actually type), and Urdu/English code-switching, not only formal Urdu.
- 79.5% pairwise win rate over the base model across two independent LLM judges on a 100-prompt, hand-curated Urdu evaluation set, up from v2's 66%, with all three of v2's regressions recovered.
- The QLoRA adapter is 154 MB, trains 0.82% of the model's parameters, and peaks at 8.56 GB of VRAM on an 80 GB H100. Fine-tuning a 7B model turned out to be far lighter than the hardware suggests.
- Evaluation is blinded pairwise judging with position-bias randomization, run across Claude, GPT, and Gemini families, plus a $0 Claude-Code-CLI judge wired in as a Haystack component.
- Code-mixed task win rate went from 0% (v1) to 80% (v2) by changing the *data*, not the hyperparameters. That was catastrophic forgetting, and it was recoverable.
- v3 trains on retrieval-format triples, which eliminated the base model's Chinese-fallback failure (45/100 answers → 0) and makes grounding safe on factual queries. Retrieval is a targeted tool here, not a blanket win.
- v3's reasoning and summarization data let Python own every checkable answer (the arithmetic, the sentence counts), and a 200-row human spot-check caught two systemic generator bugs before training.
- The demo is a free Gradio Space proxying to a Modal H100 backend, with script-aware Nastaliq/Latin rendering that detects each paragraph's script live.
- The full pipeline, evaluation scripts, and roadmap are public under Apache 2.0.
...
- The eval set is hand-curated, not scraped: 100 prompts across 8 categories (QA, summarization, translation, grammar, reasoning, creative, code explanation, code-mixed), written by me and spot-validated by another native Urdu speaker. The split is intentionally realistic: 70 Urdu-script, 25 Roman Urdu, 5 code-mixed.
- Base and fine-tuned models generate with identical decoding parameters. If the fine-tune wins, it's the weights talking, not a temperature trick.
- Judging is blinded and position-randomized. Each comparison is A/B-shuffled per item before it reaches a judge, with an explicit rule that a confident-but-wrong answer loses to one that admits uncertainty. This matters more than it sounds: I later measured that automated judges scored 8–14 percentage points higher than my earlier manual passes on the *same* outputs, almost entirely because the manual passes had a fixed base-first ordering and the automated ones randomized position. Position bias is real, and the only defense is to randomize and to use more than one judge.
- Scores come from multiple model families, reported as median plus range. For v2, Claude, GPT, and Gemini bracketed a real spread (66% median, 48–67% range), disagreeing most on code and reasoning and agreeing on translation and creative. For v3, Gemini's free tier capped out, so the two cross-model judges (Claude and GPT-5.3) carried the scoring. Reporting the spread is more honest than picking the friendliest judge.
- One judge is free, via the Claude Code CLI. I wrapped `claude -p --output-format json` as a Haystack component, calling it as a subprocess on a Max subscription with no API key. It cost nothing and let me re-judge the full set on demand.
...
- A program owns every verifiable answer. v3's reasoning and summarization data are generated with Python doing the arithmetic and the sentence counts, so the model only narrates, never computes. Deterministic checks plus a 200-row human spot-check caught two systemic generator bugs before they reached training.
...
- A 7B QLoRA tune is surprisingly light. Peaking at 8.56 GB on an 80 GB card means this kind of fine-tuning fits on hardware most people already have access to. The barrier to specializing open models is lower than it looks.
...
- Beat v2 outright, not only the base. v3 is a rebalance (43% head-to-head against v2), and reasoning (53%) is still the soft spot. The goal is targeted reasoning data that doesn't give back the summarization and grammar gains.
